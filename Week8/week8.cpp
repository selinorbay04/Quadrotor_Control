...
void set_motors_PID(data data, bool autonomous){
    ...
    if(autonomous){
        printf("auton mode \n");
        pitch_desired = PITCH_AMPLITUDE * (data.pitch - 128) / 127; //data.y;
        roll_desired = ROLL_AMPLITUDE * (data.roll - 128) / 127; //data.x;
        yaw_desired = data.camera_yaw;
    ...
    pitch_error =  (float)pitch_desired - pitch_actual;
    roll_error = (float)roll_desired - roll_actual;
    yaw_error = (float)yaw_desired - yaw_actual;
    ...
    int yaw_correction = yaw_error * p_gain_yaw;
